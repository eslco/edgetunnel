// SUBCONFIG: https://github.com/eslco/base/raw/main/config/Clash/ES_Online_MultiCountry.ini
...
function makeReadableWebSocketStream(webSocketServer, earlyDataHeader, log) {
    ...
    const stream = new ReadableStream({
        ...
        pull(controller) {
            // 这里可以实现反压机制
            // 如果 WebSocket 可以流满时停止读取，我们就可以实现反压
 ...
 * @returns {{ earlyData: ArrayBuffer | undefined, error: Error | null }} ArrayBuffer 或错误
 */
...
async function socks5Connect(addressType, addressRemote, portRemote, log) {
    ...
    // 5: S5, 2: 支持的认证方法数, 0和2: 两种认证方法（无认证和用户名/密码）
...
/**
 * 建立 HTTP 代理连接
 * @param {string} addressRemote 目标地址（ IP&域名）
 ...
 *   - "username:password@hostname:port" （认证）
 *   - "hostname:port" 不需认证
...
const cmad = decodeURIComponent(atob('aHR0cHMlM0ElMkYlMkZ0Lm1lJTJGVGdfaWRjJTBBaHR0cHMlM0ElMkYlMkZ0Lm1lJTJGQ01MaXVzc3NzJTBBR2l0aHViJTIwUmVwbyUyMFN0YXIhU3RhciFTdGFyISEhJTBBaHR0cHMlM0ElMkYlMkZnaXRodWIuY29tJTJGY21saXUlMkZlZGdldHVubmVs'));
/**
 * @param {string} userID
...
async function 生成配置信息(userID, hostName, sub, UA, RproxyIP, _url, fakeUserID, fakeHostName, env) {
    ...
    if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        ...
        const 节点配置页 = `
            ################################################################<br>
            Subscribe / URL , Click to <strong> Copy SUBURL</strong> and <strong> Generate New QR Code</strong> <br>
            ---------------------------------------------------------------<br>
            Auto Sub :<br>
            ...
            ---------------------------------------------------------------<br>
            ################################################################<br>
            ${FileName} Config Info<br>
            ---------------------------------------------------------------<br>
            ${动态UUID信息}HOST: ${hostName}<br>
            UUID: ${userID}<br>
            FKID: ${fakeUserID}<br>
            UA: ${UA}<br>
            SCV（Skip TLS Cert Varify）: ${SCV}<br>
            ${订阅器}<br>
            ---------------------------------------------------------------<br>
            ################################################################<br>
            v2ray<br>
...
async function 整理测速结果(tls) {
    ...
    for (const csvUrl of addressescsv) {
        try {
            ...
            const ipAddressIndex = 0;// IP地址在 CSV 头部
            const portIndex = 1;// 端口在 CSV 头部
            ...
            <body>
                ...
                <div class="editor-container">
                    ...
                    <div class="save-container">
                        ...
                        <button class="bestip-btn" onclick="goBestIP()"> Online BestIP </button>
                        <button class="save-btn" onclick="saveContent(this)">保存 Save </button>
...
async function getUsage(accountId, email, apikey, apitoken, all = 100000) {
    ...
    try {
        ...
        console.log(`Data Results - Pages: ${pages}, Workers: ${workers}, Sum: ${total}`);

        // 返回格式：[总限额, Pages请求数, Workers请求数, 总请求数]
        return [all, pages || 0, workers || 0, total || 0];
    } catch (error) {
        console.error('Error Occured when Acquire Usage DATA:', error.message); // 发生错误时返回默认值
        return [all, 0, 0, 0];
    }
}
